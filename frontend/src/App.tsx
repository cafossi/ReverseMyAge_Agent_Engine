...
export default function App() {
  ...
  const processSseEventData = (jsonData: string, aiMessageId: string) => {
    ...
    if (textParts.length > 0 && agent !== "report_composer_with_citations") {
      ...
      // ALWAYS update message content for ALL agents
  ...
  const handleSubmit = useCallback(async (query: string, files?: File[]) => {
    ...
    try {
      ...
      const sendMessage = async () => {
        ...
        if (files && files.length > 0) {
          ...
          formData.append('corpus_name', 'm1_hr_handbook'); // Amanda's default corpus
