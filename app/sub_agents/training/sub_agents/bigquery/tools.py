...
- **CRITICAL DATA TYPE HANDLING:** 
  * The 'course' field in APEX_NWS is FLOAT type (413.0, not 413)
  * In WHERE clauses: Use `WHERE course = 413.0` OR `WHERE CAST(course AS INT64) = 413`
  * In PARTITION BY clauses: ALWAYS use `PARTITION BY employee_id, CAST(course AS INT64)` - NEVER partition by raw FLOAT course field
  * The 'course_completion_date' field is STRING and may contain 'nan' or invalid dates
  * ALWAYS use SAFE.PARSE_DATE('%m/%d/%Y', course_completion_date) - NEVER use PARSE_DATE without SAFE prefix
  * SAFE.PARSE_DATE returns NULL for invalid dates (including 'nan') instead of causing query failures
