...
def store_results_in_context(
    ...
    if status == "SUCCESS":
        ...
        # Enhanced error logging - show full response for debugging
        print(f"[ADK][BQ] Query failed!")
        print(f"[ADK][BQ] Error: {error_message}")
        print(f"[ADK][BQ] Full tool_response: {json.dumps(tool_response, default=str, indent=2)}")
        
        tool_context.state["query_result"] = None
        tool_context.state["query_metadata"] = {
            "status": "failed",
            "error": error_message,
            "full_response": str(tool_response),  # Store full response for debugging
            "timestamp": datetime.utcnow().isoformat() + "Z",
        }
