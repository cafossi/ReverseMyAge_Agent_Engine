...
"""Top-level tools for the Touch Points (APEX_TP) multi-agent.

- call_db_agent: routes NL intent to the DB (NL2SQL) agent
- call_ds_agent: routes analysis to the DS (NL2Py) agent
- export_report_to_file: saves rendered Touch Points reports (HTML/PDF)
...
# ============================================================
# 🛠️ Tool: export_report_to_file  (Touch Points)
# ------------------------------------------------------------
# Exports Touch Points reports to PDF or HTML format.
# Keep the signature parallel to NBOT so callers can reuse flows.
# ============================================================
...
async def export_report_to_file(
    report_id: str,
    format: str,
    # Common scoping filters (parallel to NBOT so flows remain reusable)
    ...
    # Touch Points–specific optional filters
    tp_type: Optional[str] = None,        # e.g., "Incident", "Inquiry", "Escalation"
    channel: Optional[str] = None,        # e.g., "Email", "Phone", "Portal"
    priority: Optional[str] = None,       # e.g., "High", "Medium", "Low"
    owner: Optional[str] = None,          # queue/agent owner
    tool_context: Optional[ToolContext] = None
) -> dict:
    """
    Export a Touch Points report to HTML or PDF format.

    Args:
        report_id: Logical key ('tp_site_analysis', 'tp_region_analysis',
                  'tp_customer_analysis', 'tp_company_overview', 'tp_exec_report', ...)
        format: 'html' or 'pdf'
        customer_code/name, location_number, region, start_date, end_date: common scope
        tp_type/channel/priority/owner: optional TP facets

    Returns:
        dict { success, file_path, format, report_id, message }
    """
    ...
    try:
        ...
        if fmt not in ("html", "pdf"):
            return {"error": f"Invalid format: {format}. Use 'html' or 'pdf'", "success": False}

        # Build kwargs and drop None values (🔧 fixed trailing comma bug)
        kwargs = {
        ...
        # If the DS agent already produced HTML, pass it through for fidelity.
        if tool_context is not None:
            ds_out = tool_context.state.get("ds_agent_output")
            if isinstance(ds_out, dict):
                html_from_ds = ds_out.get("html") or ds_out.get("analysis_html")
                if html_from_ds:
                    kwargs["html"] = html_from_ds
        ...
        return {
            ...
            "report_id": report_id,
            "message": (
                f"✅ {report_id.replace('_', ' ').title()} generated as {fmt.upper()}.\n\n"
                f"📁 File: {file_path}\n"
                f"(Touch Points – APEX_TP)"
            ),
        }
    except Exception as e:
        return {"error": str(e), "success": False, "message": f"❌ TP export failed: {e}"}
