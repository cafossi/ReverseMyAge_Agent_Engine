...
def get_bigquery_schema_and_samples():
    ...
    For APEX_Counters specifically:
    - Gets 5 sample rows (for data structure examples)
    - Gets ALL distinct values for key columns (for "What X do we have?" queries)
    """
    ...
    for table in client.list_tables(dataset_ref):
        ...
        # 🔥 NEW: For APEX_Counters, load ALL distinct values for key columns
        if table_id == "APEX_Counters":
            logging.info(f"📊 Loading complete distinct values for {table_id}")
            ...
            for column in key_columns:
                try:
                    distinct_query = f"""
                        ...
                        ORDER BY {column}
                    """
                    distinct_df = client.query(distinct_query).to_dataframe()
                    distinct_values = distinct_df[column].tolist()
                    
                    # Serialize for SQL
                    sample_values[column] = [_serialize_value_for_sql(v) for v in distinct_values]
                    
                    logging.info(f"  ✅ {column}: {len(distinct_values)} distinct values")
                except Exception as e:
                    logging.warning(f"  ⚠️ Failed to load {column}: {e}")
