...
def get_bigquery_schema_and_samples():
    ...
    # CRITICAL SCHEMA RESTRICTION: Only allow APEX_NWS table for scheduling analysis
    # This prevents the NL2SQL tool from discovering and using other tables in the dataset
    # (like customer_location, employees, schedules) which don't contain the denormalized 
    # scheduling data we need. All scheduling analysis must use the single APEX_NWS table
    # which contains employee, location, customer, and scheduling data in one flat structure
    
    allowed_tables = ['APEX_NWS']
    
    for table in client.list_tables(dataset_ref):
        # Skip tables that aren't in the allowed list
        if table.table_id not in allowed_tables:
            continue
...
- **CRITICAL - No Joins:** The schema contains ONLY flat, denormalized tables. NEVER use JOIN, LEFT JOIN, RIGHT JOIN, INNER JOIN, or any join operation. All necessary data is already in a single table. If you think you need to join tables, you are mistaken - check the schema again.
